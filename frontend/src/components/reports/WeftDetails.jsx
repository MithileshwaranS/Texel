...
const WeftDetails = ({ design, wefts }) => {
  ...
  const handleExportPDF = async () => {
    handleMenuClose();
    const content = document.getElementById("weft-details-content");
    ...
    try {
      ...
      pdf.save(`${design.designname || "weft"}-details.pdf`);
    } catch (err) {
  ...
  return (
    <Box id="weft-details-content">
      ...
      <Box sx={{ mb: 4 }}>
        ...
        >
          Pick: {wefts[0]?.pick || "N/A"} | Total Width:{" "}
          {wefts[0]?.width || "N/A"}
        </Typography>
      </Box>
              ...
              <TableBody>
                {weft.colors?.map((color) => (
                  <TableRow key={color.id} hover>
                    ...
                    <TableCell align="right">
                      {((color.threads / weft.totalthreads) * 100).toFixed(2)}%
                    </TableCell>
                  </TableRow>
                ))}
                <TotalRow>
                  ...
                  <TableCell sx={{ fontWeight: 600 }} align="right">
                    {weft.totalthreads.toLocaleString()}
                  </TableCell>
                  <TableCell sx={{ fontWeight: 600 }} align="right">
                    {parseFloat(weft.weftweight).toFixed(3)}
                  </TableCell>
                  <TableCell sx={{ fontWeight: 600 }} align="right">
                    {parseFloat(weft.ordertotalweight).toFixed(3)}
                  </TableCell>
                  <TableCell sx={{ fontWeight: 600 }} align="right">
                    100%
                  </TableCell>
