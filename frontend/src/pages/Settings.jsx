import { FaCog, FaPlus, FaTimes, FaEdit, FaTrash, FaSearch } from "react-icons/fa";
...
function Settings() {
  ...
  const handleAddYarn = async () => {
    ...
    try {
      const endpoint = isEditing
        ? `https://texel.onrender.com/api/editYarn/${currentId}`
        : "https://texel.onrender.com/api/addYarn";
  ...
  const handleDeleteYarn = async (id) => {
    if (window.confirm("Are you sure you want to delete this yarn?")) {
      try {
        const response = await fetch(`https://texel.onrender.com/api/deleteYarn/${id}`, {
          method: "DELETE"
        });
  ...
  const fetchYarnData = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("https://texel.onrender.com/api/yarnDetails");
        ...
        >
          ...
          <div className="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center">
              ...
              <h2 className="text-lg font-semibold text-gray-800">Yarn Details</h2>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-gray-700">
                  {searchTerm ? 'No matching yarns found' : 'No yarn details found'}
                </h3>
                <p className="mt-1 text-gray-500">
                  {searchTerm ? 'Try a different search term' : 'Add your first yarn to get started'}
                </p>
                {!searchTerm && (
                  <button
                    onClick={handleOpenDialog}
                  ...
                  <TableHead>
                    <TableRow className="bg-gradient-to-r from-gray-50 to-gray-100">
                      {headers.map((header, index) => (
                        ...
                        >
                          {header === 'actions' ? 'Actions' : header.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
                        </TableCell>
                      ))}
                    </TableRow>
                  </TableHead>
