...
app.get("/api/designdetails/:id", async (req, res) => {
  const { id } = req.params;
  const { data, error } = await supabase.rpc("get_design_detail_by_id", {
...
app.put("/api/editYarn/:id", async (req, res) => {
  ...
  const { data, error } = await supabase
    .from("yarndetails")
    .update({ yarn_count: yarnCount, hanks_wt: hanksWt, yarnprice: yarnPrice })
    .eq("id", id)
    .select();
  if (error)
    return res
...
app.post("/api/submit", async (req, res) => {
  ...
  try {
    ...
    const { data: designInsert, error: insertErr } = await supabase
      .from("designs")
      .insert([
        {
    ...
    const designId = designInsert[0].design_id;
    ...
    for (let i = 0; i < body.wefts.length; i++) {
      ...
      const { error } = await supabase.from("wefts").insert([
        {
          design_id: designId,
          weftcount: wefts.count,
          weftweight: weftWeight,
          initweftcost: wefts.cost,
          weftdyeing: wefts.dyeing,
          pick: parseFloat(wefts.pick),
        },
      ]);
      if (error) throw error;
    }
