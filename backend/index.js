...
app.post("/api/submit", async (req, res) => {
  ...
  try {
    ...
    const { data: insertDesign, error: insertErr } = await supabase
      .from("designs")
      .insert([
        {
    ...
    for (let i = 0; i < body.wefts.length; i++) {
      ...
      await supabase.from("wefts").insert([
        {
          design_id: designId,
